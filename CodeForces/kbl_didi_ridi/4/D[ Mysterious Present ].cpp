...
<head>
    ...
    <meta name="X-Csrf-Token" content="7ff7364319199e721633cc4a840f3ad0"/>
    ...
    <meta property="og:title" content="Submission #16397102 - Codeforces"/>
    ...
    <title>Submission #16397102 - Codeforces</title>
...
<body><span style='display:none;' class='csrf-token' data-csrf='7ff7364319199e721633cc4a840f3ad0'>&nbsp;</span>
...
<div id="header" style="position: relative;">
    ...
    <div class="lang-chooser">
        ...
        <div >
                        <a href="/enter?back=%2Fcontest%2F4%2Fsubmission%2F16397102">Enter</a>
                     | 
    ...
    <ul class="menu-list main-menu-list">
        ...
        <form method="post" action="/search"><input type='hidden' name='csrf_token' value='7ff7364319199e721633cc4a840f3ad0'/>
...
<a href="/profile/kbl_didi_ridi" title="Expert kbl_didi_ridi" class="rated-user user-blue">kbl_didi_ridi</a>    </td>
    <td>
        <a title="D - Mysterious Present" href="/contest/4/problem/D">4D</a>
...
int main()
{
	int n,w,h;
	cin>>n>>w>>h;
	a.push_back( make_pair( make_pair(w,h) , 0 ) );
	for(int i=1;i<=n;i++)
	{
		int x,y;
		cin>>x>>y;
...
/*	cerr<<"------------------------"<<endl;
	for(int i=1;i<n;i++)
		cerr<<a[i].f.f<<" "<<a[i].f.s<<" "<<a[i].s<<endl;
	cerr<<"------------------------"<<endl;*/
	d[0]=0;
	for(int i=1;i<n;i++)
	{
	...
	while(ind!=0)
	{
		answ.push_back(a[ind].s);
		for(int i=ind-1;0<=i;i--)
			if(d[ind]==d[i]+1 && a[ind].f.f>a[i].f.f && a[ind].f.s>a[i].f.s )
			{
				ind=i;
				break;
			}
	}
	for(int i=answ.size()-1;0<=i;i--)
		cout<<answ[i]<<" ";
	cout<<endl;
}