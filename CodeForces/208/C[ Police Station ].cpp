...
<head>
    ...
    <meta name="X-Csrf-Token" content="d797b951bca833afd00133cdf1503dea"/>
    ...
    <meta property="og:title" content="Submission #14509517 - Codeforces"/>
    ...
    <title>Submission #14509517 - Codeforces</title>
...
<body><span style='display:none;' class='csrf-token' data-csrf='d797b951bca833afd00133cdf1503dea'>&nbsp;</span>
...
<div id="header" style="position: relative;">
    ...
    <div class="lang-chooser">
        ...
        <div >
                        <a href="/enter?back=%2Fcontest%2F208%2Fsubmission%2F14509517">Enter</a>
                     | 
    ...
    <ul class="menu-list main-menu-list">
        ...
        <form method="post" action="/search"><input type='hidden' name='csrf_token' value='d797b951bca833afd00133cdf1503dea'/>
...
<a href="/profile/kbl_didi_ridi" title="Expert kbl_didi_ridi" class="rated-user user-blue">kbl_didi_ridi</a>    </td>
    <td>
        <a title="C - Police Station" href="/contest/208/problem/C">208C</a>
...
void bfs(int v)
{
    ...
    while (!q.empty())
    {
        ...
        for (int i = 0; i < edge[u].size(); i++)
            if (!mrk[edge[u][i]])
            {
                q.push(edge[u][i]); mrk[edge[u][i]] = 1;
                h[edge[u][i]] = h[u] + 1;
            }
        q.pop();
    }
...
int main()
{
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        edge[num1].push_back(num2); edge[num2].push_back(num1);
    }
    d[u][0] = 1;
    for (int i = 0; i < edge[u].size(); i++)
        d[edge[u][i]][1] = 1;
    ...
    for (int i = 0; i < n; i++)
        for (int j = 0; j <= dis; j++)
            cnt[i] += d[i][j] * d_2[i][dis - j];
    long long int max_cnt = max(cnt[u], cnt[v]);
    for (int i = 0; i < n; i++)
        if (i != u && i != v)
            max_cnt = max(max_cnt, 2 * cnt[i]);
    if (d[v][dis] != 0)
        cout << setprecision(10) << (double)max_cnt / d[v][dis] << endl;
    else
        cout << 2 << endl;
    return 0;
}
