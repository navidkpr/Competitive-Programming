...
<head>
    ...
    <meta name="X-Csrf-Token" content="981f2126a72990613da0e12ac27a95dd"/>
    ...
    <meta property="og:title" content="Submission #16017196 - Codeforces"/>
    ...
    <meta name="cc" content="15a5db6d563f4618ee3f9db6f50940c0f59645dd"/>
    
    
    <meta name="utc_offset" content="+03:00"/>
    <meta name="verify-reformal" content="f56f99fd7e087fb6ccb48ef2" />
    <title>Submission #16017196 - Codeforces</title>
...
<body><span style='display:none;' class='csrf-token' data-csrf='981f2126a72990613da0e12ac27a95dd'>&nbsp;</span>
...
<div id="header" style="position: relative;">
    ...
    <div class="lang-chooser">
        ...
        <div >
                        <a href="/enter?back=%2Fcontest%2F52%2Fsubmission%2F16017196">Enter</a>
                     | 
    ...
    <ul class="menu-list main-menu-list">
        ...
        <form method="post" action="/search"><input type='hidden' name='csrf_token' value='981f2126a72990613da0e12ac27a95dd'/>
...
<a href="/profile/kbl_didi_ridi" title="Expert kbl_didi_ridi" class="rated-user user-blue">kbl_didi_ridi</a>    </td>
    <td>
        <a title="C - Circular RMQ" href="/contest/52/problem/C">52C</a>
         - <span title="problem revision">2</span>
    </td>
    <td>
    GNU C++
    </td>
    ...
    <pre id="program-source-text" class="prettyprint lang-cpp program-source" style="padding: 0.5em;">//ITNOG
#include <bits/stdc++.h>
...
void update(int ind)
{
    if(seg[2 * ind] + ad[2 * ind] <= seg[2 * ind + 1] + ad[2 * ind + 1])
	  seg[ind] = seg[2 * ind + 0] + ad[2 * ind + 0];
    else
	  seg[ind] = seg[2 * ind + 1] + ad[2 * ind + 1];
}

void add(int l, int r, int val, int s, int e, int ind)
{
    if(l >= e || r <= s) return;
    if(l <= s && e <= r) { ad[ind] += val; return; }
    
    int mid = (s + e) >> 1;
    
    shift(ind);
    add(l, r, val, s, mid, 2 * ind + 0);
...
{								
    ...
    if(l <= s && e <= r) { return seg[ind] + ad[ind]; }
...
main()
{
    ...
    int n; cin >> n;
    for(int i = 0; i < n; i ++)
	  cin >> a[i], add(i, i + 1, a[i], 0, n, 1);
    
    int q; cin >> q; cin.ignore();
    while(q --)
    {
	  string s; getline(cin, s);
	  vector<int> res; int cur = 0, fl = 0;
	  for(int i = 0; i < s.size(); i ++)
	  {
		if(s[i] == &#39; &#39;) { res.push_back(cur); cur = 0; continue; }
	  ...
	  int l = res[0], r = res[1]; r ++; //cout << l << &#39; &#39; << r << endl;
	  if(res.size() == 2)
	  {
		if(l < r)
		{
		    cout << get(l, r, 0, n, 1) << endl; continue;
		}
		else
		{
		    int ans = get(l, n, 0, n, 1);
		    ans = min(ans, get(0, r, 0, n, 1));
